...
Value nr_create_small_object(Value self, Value _v0, Value _v1, Value _v2, Value _v3, Value _v4);

Value nr_v_create_small_object;
Value nr_v_id;
Value nr_v_print;
Value nr_v_iterations;
Value nr_v_batch_size;
long long nr_v_i;
Value nr_v_batch;
long long nr_v_j;
Value nr_v_first;
...
Value nr_create_small_object(Value self, Value _v0, Value _v1, Value _v2, Value _v3, Value _v4) {
  ...
  return ({ Value _o = val_obj(); set_field(_o, "id", nr_v_id); set_field(_o, "type", val_str("test")); set_field(_o, "value", val_int(((nr_v_id).data.i * (val_float(1.5)).data.i))); set_field(_o, "flag", val_bool(true)); set_field(_o, "data", val_str("some random string data to fill memory")); _o; });
  return val_nil();
}
int main(int argc, char** argv) {
  ...
  nr_v_create_small_object = val_nil();
  nr_v_id = val_nil();
  nr_v_print = val_nil();
  nr_v_iterations = val_nil();
  nr_v_batch_size = val_nil();
  nr_v_i = 0;
  nr_v_batch = val_nil();
  nr_v_j = 0;
  nr_v_first = val_nil();
  ...
  nr_v_create_small_object = val_func(nr_create_small_object);
val_func(nr_create_small_object);
val_nil();
({ nr_rt_print(val_nil(), val_str("=== MASSIVE ALLOCATION TEST ==="), val_nil(), val_nil(), val_nil(), val_nil()); val_nil(); });
  nr_v_iterations = val_int(50);
  nr_v_batch_size = val_int(1000);
  nr_v_i = 0LL;
  while ((nr_v_i < (nr_v_iterations).data.i)) {
({ nr_rt_print(val_nil(), nr_rt_add(nr_rt_add(nr_rt_add(val_str("Batch "), nr_rt_add(val_int(nr_v_i), val_int(1))), val_str(" / ")), nr_v_iterations), val_nil(), val_nil(), val_nil(), val_nil()); val_nil(); });
  nr_v_batch = ({ Value _a = val_arr(); _a; });
  nr_v_j = 0LL;
  while ((nr_v_j < (nr_v_batch_size).data.i)) {
({ nr_rt_push(val_nil(), nr_v_batch, nr_create_small_object(val_nil(), nr_rt_add(val_int((nr_v_i * (nr_v_batch_size).data.i)), val_int(nr_v_j)), val_nil(), val_nil(), val_nil(), val_nil()), val_nil(), val_nil(), val_nil()); val_nil(); });
  nr_v_j = (nr_v_j + 1LL);
  }
;
  nr_v_first = nr_rt_at(val_nil(), nr_v_batch, val_int(0), val_nil(), val_nil(), val_nil());
  nr_v_last = nr_rt_at(val_nil(), nr_v_batch, val_int(((nr_v_batch_size).data.i - 1LL)), val_nil(), val_nil(), val_nil());
  nr_v_i = (nr_v_i + 1LL);
  }
;
({ nr_rt_print(val_nil(), val_str("=== ALLOCATION TEST PASSED ==="), val_nil(), val_nil(), val_nil(), val_nil()); val_nil(); });
  return 0; 
}
