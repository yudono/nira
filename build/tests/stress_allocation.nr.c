...
Value nr_create_small_object(Value self, Value _v0, Value _v1, Value _v2, Value _v3, Value _v4) {
  ...
  return ({ Value _o = val_obj(); set_field(_o, "id", nr_v_id); set_field(_o, "type", val_str("test")); set_field(_o, "value", val_int(((nr_v_id).data.i * (((Value){.type = VAL_FLOAT, .data.f = 1.5})).data.i))); set_field(_o, "flag", val_bool(true)); set_field(_o, "data", val_str("some random string data to fill memory")); _o; });
  return val_nil();
}
