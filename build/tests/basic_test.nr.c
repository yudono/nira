...
val_nil();
({ nr_rt_print(val_nil(), val_str("--- Basic Test ---"), val_nil(), val_nil(), val_nil(), val_nil()); val_nil(); });
  nr_v_a = val_int(10);
  nr_v_b = val_int(20);
({ nr_rt_print(val_nil(), nr_rt_add(nr_v_a, nr_v_b), val_nil(), val_nil(), val_nil(), val_nil()); val_nil(); });
  { const char* _lit = "Hello"; nr_v_s_len = strlen(_lit); memcpy(nr_v_s, _lit, nr_v_s_len); nr_v_s[nr_v_s_len] = 0; };
({ nr_rt_print(val_nil(), nr_rt_add(val_str_len(nr_v_s, nr_v_s_len), val_str(" World")), val_nil(), val_nil(), val_nil(), val_nil()); val_nil(); });
  if (((nr_v_a).data.i < (nr_v_b).data.i)) {
({ nr_rt_print(val_nil(), val_str("a < b"), val_nil(), val_nil(), val_nil(), val_nil()); val_nil(); });
  };
  return 0; 
}
