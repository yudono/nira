...
val_func(nr_main);
  nr_v_x = val_int(10);
  nr_v_f = val_func(nr_anonymous);
({ nr_rt_print(val_nil(), val_str("Testing closure:"), val_nil(), val_nil(), val_nil(), val_nil()); val_nil(); });
({ nr_rt_print(val_nil(), ({ Value _f = nr_v_f; if(_f.type==VAL_FUNC) { ((Value (*)(Value, Value, Value, Value, Value, Value))_f.data.func_ptr)(val_nil(), val_nil(), val_nil(), val_nil(), val_nil(), val_nil()); } else { val_nil(); } }), val_nil(), val_nil(), val_nil(), val_nil()); val_nil(); });
