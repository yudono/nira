...
({ nr_rt_print(val_nil(), val_str("HTTP module loaded"), val_nil(), val_nil(), val_nil(), val_nil()); val_nil(); });
  nr_v_app = ({ Value _f = get_field(nr_v_http, "app"); _f.type==VAL_FUNC ? ((Value (*)(Value, Value, Value, Value, Value, Value))_f.data.func_ptr)(nr_v_http, val_nil(), val_nil(), val_nil(), val_nil(), val_nil()) : val_nil(); });
