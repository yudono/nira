...
({ nr_rt_print(val_nil(), val_str("HTTP module loaded"), val_nil(), val_nil(), val_nil(), val_nil()); val_nil(); });
  nr_v_app = ({ Value _f = get_field(nr_v_http, "app"); if(_f.type==VAL_FUNC) { ((Value (*)(Value, Value, Value, Value, Value, Value))_f.data.func_ptr)(nr_v_http, val_nil(), val_nil(), val_nil(), val_nil(), val_nil()); } else if (0) { val_nil(); } else { val_nil(); } });
