...
int main(int argc, char** argv) {
  ...
  { Value _m = nr_rt_load_module("math"); nr_v_max = get_field(_m, "max"); nr_v_min = get_field(_m, "min"); }
;
val_func(nr_main);
({ nr_rt_print(val_nil(), ({ Value _f = nr_v_max; if(_f.type==VAL_FUNC) { ((Value (*)(Value, Value, Value, Value, Value, Value))_f.data.func_ptr)(val_nil(), val_int(10), val_int(20), val_nil(), val_nil(), val_nil()); } else { val_nil(); } }), val_nil(), val_nil(), val_nil(), val_nil()); val_nil(); });
({ nr_rt_print(val_nil(), ({ Value _f = nr_v_min; if(_f.type==VAL_FUNC) { ((Value (*)(Value, Value, Value, Value, Value, Value))_f.data.func_ptr)(val_nil(), val_int(10), val_int(20), val_nil(), val_nil(), val_nil()); } else { val_nil(); } }), val_nil(), val_nil(), val_nil(), val_nil()); val_nil(); });
  return 0; 
}
