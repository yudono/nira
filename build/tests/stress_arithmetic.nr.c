...
Value nr_harmonic_sum(Value self, Value _v0, Value _v1, Value _v2, Value _v3, Value _v4) {
  long long nr_v_i=0, nr_v_j=0;
  nr_v_n = _v0; if(nr_v_n.type == VAL_NIL) { }
  nr_v_total = ((Value){.type = VAL_FLOAT, .data.f = 0});
  nr_v_i = 1LL;
  while ((nr_v_i <= (nr_v_n).data.i)) {
  nr_v_total = nr_rt_add(nr_v_total, val_int(((((Value){.type = VAL_FLOAT, .data.f = 1})).data.i / nr_v_i)));
  nr_v_i = (nr_v_i + 1LL);
  }
;
  return nr_v_total;
...
({ nr_rt_print(val_nil(), val_str("Float Precision Tests:"), val_nil(), val_nil(), val_nil(), val_nil()); val_nil(); });
  nr_v_a = nr_rt_add(((Value){.type = VAL_FLOAT, .data.f = 0.1}), ((Value){.type = VAL_FLOAT, .data.f = 0.2}));
({ nr_rt_print(val_nil(), nr_rt_add(val_str("0.1 + 0.2 = "), nr_v_a), val_nil(), val_nil(), val_nil(), val_nil()); val_nil(); });
  nr_v_b = val_int(((((Value){.type = VAL_FLOAT, .data.f = 1})).data.i / (((Value){.type = VAL_FLOAT, .data.f = 3})).data.i));
