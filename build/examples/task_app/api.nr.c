...
typedef enum { VAL_NIL, VAL_INT, VAL_STR, VAL_OBJ, VAL_ARR, VAL_BOOL, VAL_FUNC, VAL_ERROR } ValueType;
struct Value; typedef struct Value { ValueType type; union { int i; char* s; struct { char** keys; struct Value** values; int count; int capacity; }* obj; struct { struct Value** elements; int count; int capacity; }* arr; void* func_ptr; } data; } Value;
...
Value val_obj() { Value v; v.type = VAL_OBJ; v.data.obj = malloc(sizeof(*v.data.obj)); v.data.obj->count = 0; v.data.obj->capacity = 8; v.data.obj->keys = malloc(sizeof(char*) * 8); v.data.obj->values = malloc(sizeof(Value*) * 8); return v; }
Value val_arr() { Value v; v.type = VAL_ARR; v.data.arr = malloc(sizeof(*v.data.arr)); v.data.arr->count = 0; v.data.arr->capacity = 8; v.data.arr->elements = malloc(sizeof(Value*) * 8); return v; }

void set_field(Value obj, const char* key, Value val) { if (obj.type != VAL_OBJ) return; for(int i=0; i<obj.data.obj->count; i++) { if(strcmp(obj.data.obj->keys[i], key) == 0) { *obj.data.obj->values[i] = val; return; } } if(obj.data.obj->count >= obj.data.obj->capacity) { obj.data.obj->capacity *= 2; obj.data.obj->keys = realloc(obj.data.obj->keys, sizeof(char*) * obj.data.obj->capacity); obj.data.obj->values = realloc(obj.data.obj->values, sizeof(Value*) * obj.data.obj->capacity); } obj.data.obj->keys[obj.data.obj->count] = strdup(key); obj.data.obj->values[obj.data.obj->count] = malloc(sizeof(Value)); *obj.data.obj->values[obj.data.obj->count] = val; obj.data.obj->count++; }
...
Value nr_rt_to_int(Value v) { if (v.type == VAL_INT) return v; if (v.type == VAL_STR) return val_int(atoi(v.data.s)); return val_int(0); }
...
Value nr_rt_push(Value arr, Value val) { if (arr.type != VAL_ARR) return val_nil(); if (arr.data.arr->count >= arr.data.arr->capacity) { arr.data.arr->capacity *= 2; arr.data.arr->elements = realloc(arr.data.arr->elements, sizeof(Value*) * arr.data.arr->capacity); } arr.data.arr->elements[arr.data.arr->count] = malloc(sizeof(Value)); *arr.data.arr->elements[arr.data.arr->count] = val; arr.data.arr->count++; return val; }
...
Value nr_rt_at(Value v, Value idx) { if (idx.type != VAL_INT) return val_nil(); int i = idx.data.i; if (v.type == VAL_ARR) { if (i < 0 || i >= v.data.arr->count) return val_nil(); return *v.data.arr->elements[i]; } if (v.type == VAL_STR) { if (i < 0 || i >= (int)strlen(v.data.s)) return val_nil(); char s[2] = {v.data.s[i], 0}; return val_str(s); } return val_nil(); }

Value nr_rt_file_read(Value path) { if (path.type != VAL_STR) return val_nil(); FILE* f = fopen(path.data.s, "r"); if (!f) return val_nil(); fseek(f, 0, SEEK_END); long size = ftell(f); fseek(f, 0, SEEK_SET); char* b = malloc(size + 1); fread(b, 1, size, f); b[size] = 0; fclose(f); Value res = val_str(b); free(b); return res; }
Value nr_rt_file_write(Value path, Value content) { if (path.type != VAL_STR || content.type != VAL_STR) return val_nil(); FILE* f = fopen(path.data.s, "w"); if (!f) return val_nil(); fputs(content.data.s, f); fclose(f); return val_int(1); }
Value nr_rt_file_exists(Value path) { if (path.type != VAL_STR) return val_bool(false); struct stat st; return val_bool(stat(path.data.s, &st) == 0); }
Value nr_rt_file_delete(Value path) { if (path.type != VAL_STR) return val_nil(); remove(path.data.s); return val_int(1); }
...
Value nr_rt_exec(Value cmd) { if (cmd.type != VAL_STR) return val_int(-1); return val_int(system(cmd.data.s)); }
Value nr_rt_exit(Value code) { if (code.type == VAL_INT) exit(code.data.i); else exit(0); return val_nil(); }
...
Value nr_rt_json_encode(Value v) { char* s = val_to_json(v); Value res = val_str(s); free(s); return res; }
  ...
  sqlite3_stmt* stmt; if (sqlite3_prepare_v2(h, sql.data.s, -1, &stmt, 0) == SQLITE_OK) {
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
      ...
      for (int i=0; i<cols; i++) {
        ...
        else v = val_str((const char*)sqlite3_column_text(stmt, i));
        set_field(o, name, v);
      }
      nr_rt_push(a, o);
    }
...
Value nr_rt_http_serve(Value port, Value callback) {
  ...
  while(1) { new_socket = accept(server_fd, (struct sockaddr *)&address, (socklen_t*)&addrlen);
    char buffer[30000] = {0}; int n = recv(new_socket, buffer, 30000, 0);
    if (n <= 0) { close(new_socket); continue; }
    char method[16], path[256]; sscanf(buffer, "%s %s", method, path);
...
Value nr_get_tasks(Value self, Value nr_v_a1, Value nr_v_a2, Value nr_v_a3, Value nr_v_a4, Value nr_v_a5);
Value nr_get_task(Value self, Value nr_v_a1, Value nr_v_a2, Value nr_v_a3, Value nr_v_a4, Value nr_v_a5);
Value nr_create_task(Value self, Value nr_v_a1, Value nr_v_a2, Value nr_v_a3, Value nr_v_a4, Value nr_v_a5);
Value nr_update_task(Value self, Value nr_v_a1, Value nr_v_a2, Value nr_v_a3, Value nr_v_a4, Value nr_v_a5);
Value nr_delete_task(Value self, Value nr_v_a1, Value nr_v_a2, Value nr_v_a3, Value nr_v_a4, Value nr_v_a5);
Value nr_v_http;
Value nr_v_string;
...
Value nr_v_app;
Value nr_v_get_tasks;
Value nr_v_get_task;
Value nr_v_t;
Value nr_v_create_task;
Value nr_v_update_task;
Value nr_v_delete_task;
...
nr_task_prepare(val_nil(), val_nil(), val_nil(), val_nil(), val_nil(), val_nil());
  return ({ Value _t = nr_v__db; Value _f = get_field(_t, "query"); Value _r; if (_f.type == VAL_FUNC) _r = ((Value (*)(Value, Value, Value, Value, Value, Value))_f.data.func_ptr)(_t, val_str("SELECT * FROM tasks"), val_nil(), val_nil(), val_nil(), val_nil()); else { _r = val_nil(); } _r; });
...
nr_task_prepare(val_nil(), val_nil(), val_nil(), val_nil(), val_nil(), val_nil());
  return ({ Value _t = nr_v__db; Value _f = get_field(_t, "query"); Value _r; if (_f.type == VAL_FUNC) _r = ((Value (*)(Value, Value, Value, Value, Value, Value))_f.data.func_ptr)(_t, val_str("SELECT * FROM tasks WHERE id = ?"), ({ Value _a = val_arr(); nr_rt_push(_a, nr_rt_to_int(nr_v_id)); _a; }), val_nil(), val_nil(), val_nil()); else { _r = val_nil(); } _r; });
...
nr_task_prepare(val_nil(), val_nil(), val_nil(), val_nil(), val_nil(), val_nil());
({ Value _t = nr_v__db; Value _f = get_field(_t, "exec"); Value _r; if (_f.type == VAL_FUNC) _r = ((Value (*)(Value, Value, Value, Value, Value, Value))_f.data.func_ptr)(_t, val_str("INSERT INTO tasks (title, done, created_at) VALUES (?, 0, ?)"), ({ Value _a = val_arr(); nr_rt_push(_a, nr_v_title); nr_rt_push(_a, nr_rt_now()); _a; }), val_nil(), val_nil(), val_nil()); else { _r = val_nil(); } _r; });
...
nr_task_prepare(val_nil(), val_nil(), val_nil(), val_nil(), val_nil(), val_nil());
({ Value _t = nr_v__db; Value _f = get_field(_t, "exec"); Value _r; if (_f.type == VAL_FUNC) _r = ((Value (*)(Value, Value, Value, Value, Value, Value))_f.data.func_ptr)(_t, val_str("UPDATE tasks SET title = ?, done = ? WHERE id = ?"), ({ Value _a = val_arr(); nr_rt_push(_a, nr_v_title); nr_rt_push(_a, nr_v_done); nr_rt_push(_a, nr_rt_to_int(nr_v_id)); _a; }), val_nil(), val_nil(), val_nil()); else { _r = val_nil(); } _r; });
...
Value nr_task_delete(Value self, Value nr_v_a1, Value nr_v_a2, Value nr_v_a3, Value nr_v_a4, Value nr_v_a5) {
  Value nr_v_id = nr_v_a1;
nr_task_prepare(val_nil(), val_nil(), val_nil(), val_nil(), val_nil(), val_nil());
({ Value _t = nr_v__db; Value _f = get_field(_t, "exec"); Value _r; if (_f.type == VAL_FUNC) _r = ((Value (*)(Value, Value, Value, Value, Value, Value))_f.data.func_ptr)(_t, val_str("DELETE FROM tasks WHERE id = ?"), ({ Value _a = val_arr(); nr_rt_push(_a, nr_rt_to_int(nr_v_id)); _a; }), val_nil(), val_nil(), val_nil()); else { _r = val_nil(); } _r; });
...
int main(int argc, char** argv) {
  ...
  nr_v_app = val_nil();
  nr_v_get_tasks = val_nil();
  nr_v_get_task = val_nil();
  nr_v_t = val_nil();
  nr_v_create_task = val_nil();
  nr_v_update_task = val_nil();
  nr_v_delete_task = val_nil();
  nr_http_init();
  if (nr_v_http.type == VAL_NIL) nr_v_http = val_obj();
  set_field(nr_v_http, "app", val_func(nr_http_app));
  set_field(nr_v_http, "serve", val_func(nr_http_serve));
...
;
;
val_func(nr_get_tasks);
val_func(nr_get_task);
val_func(nr_create_task);
val_func(nr_update_task);
val_func(nr_delete_task);
...
({ Value _t = nr_v_app; Value _f = get_field(_t, "get"); Value _r; if (_f.type == VAL_FUNC) _r = ((Value (*)(Value, Value, Value, Value, Value, Value))_f.data.func_ptr)(_t, val_str("/tasks"), val_func(nr_get_tasks), val_nil(), val_nil(), val_nil()); else { _r = val_nil(); } _r; });
({ Value _t = nr_v_app; Value _f = get_field(_t, "get"); Value _r; if (_f.type == VAL_FUNC) _r = ((Value (*)(Value, Value, Value, Value, Value, Value))_f.data.func_ptr)(_t, val_str("/tasks/:id"), val_func(nr_get_task), val_nil(), val_nil(), val_nil()); else { _r = val_nil(); } _r; });
({ Value _t = nr_v_app; Value _f = get_field(_t, "post"); Value _r; if (_f.type == VAL_FUNC) _r = ((Value (*)(Value, Value, Value, Value, Value, Value))_f.data.func_ptr)(_t, val_str("/tasks"), val_func(nr_create_task), val_nil(), val_nil(), val_nil()); else { _r = val_nil(); } _r; });
({ Value _t = nr_v_app; Value _f = get_field(_t, "put"); Value _r; if (_f.type == VAL_FUNC) _r = ((Value (*)(Value, Value, Value, Value, Value, Value))_f.data.func_ptr)(_t, val_str("/tasks/:id"), val_func(nr_update_task), val_nil(), val_nil(), val_nil()); else { _r = val_nil(); } _r; });
({ Value _t = nr_v_app; Value _f = get_field(_t, "delete"); Value _r; if (_f.type == VAL_FUNC) _r = ((Value (*)(Value, Value, Value, Value, Value, Value))_f.data.func_ptr)(_t, val_str("/tasks/:id"), val_func(nr_delete_task), val_nil(), val_nil(), val_nil()); else { _r = val_nil(); } _r; });
