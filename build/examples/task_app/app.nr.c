...
Value nr_cli_run(Value self, Value nr_v_a1, Value nr_v_a2, Value nr_v_a3, Value nr_v_a4, Value nr_v_a5) {
  ...
  nr_v_title = nr_rt_at(nr_v_args, val_int(2));
...
nr_rt_print(nr_rt_add(nr_rt_add(nr_rt_add(nr_rt_add(nr_v_status, val_str(" ")), get_field(nr_v_t, "id")), val_str(": ")), get_field(nr_v_t, "title")));
  }
  }
;
  };
  if (is_truthy(nr_rt_eq(nr_v_cmd, val_str("done")))) {
  nr_v_id = nr_rt_at(nr_v_args, val_int(2));
({ Value _t = nr_v_task; Value _f = get_field(_t, "markDone"); Value _r; if (_f.type == VAL_FUNC) _r = ((Value (*)(Value, Value, Value, Value, Value, Value))_f.data.func_ptr)(_t, nr_v_id, val_nil(), val_nil(), val_nil(), val_nil()); else { _r = val_nil(); } _r; });
...
({ Value _t = nr_v_api; Value _f = get_field(_t, "start"); Value _r; if (_f.type == VAL_FUNC) _r = ((Value (*)(Value, Value, Value, Value, Value, Value))_f.data.func_ptr)(_t, val_nil(), val_nil(), val_nil(), val_nil(), val_nil()); else { _r = val_nil(); } _r; });
  }
;
; return 0; }
