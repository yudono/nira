...
int main(int argc, char** argv) {
  ...
  nr_v_i = val_nil();
;
nr_rt_print(val_str("Testing Arena Allocator..."));
  nr_v_a = ({ Value _o = val_obj(); set_field(_o, "x", val_int(10)); set_field(_o, "y", val_int(20)); _o; });
nr_rt_print(get_field(nr_v_a, "x"));
nr_rt_print(get_field(nr_v_a, "y"));
  nr_v_arr = ({ Value _a = val_arr(); nr_rt_push(_a, val_int(1)); nr_rt_push(_a, val_int(2)); nr_rt_push(_a, val_int(3)); _a; });
  if (nr_v_arr.type == VAL_ARR) {
  for (int _i1 = 0; _i1 < nr_v_arr.data.arr->count; _i1++) {
    nr_v_i = *nr_v_arr.data.arr->elements[_i1];
nr_rt_print(nr_v_i);
  }
  }
;
nr_rt_print(val_str("Arena test passed if no crash!"));
