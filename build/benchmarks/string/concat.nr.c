...
;
  ...
  nr_v_s = val_str("");
  nr_v_i = val_int(0);
  while (nr_v_i.data.i < val_int(100000).data.i) {
  nr_v_s = nr_rt_add(nr_v_s, val_str("a"));
...
nr_rt_print(nr_rt_add(nr_rt_add(nr_rt_add(nr_rt_add(val_str("Nira: "), nr_rt_to_string(val_int(nr_v_end.data.i - nr_v_start.data.i))), val_str(" ms (Length: ")), nr_rt_to_string(nr_rt_len(nr_v_s))), val_str(")")));
