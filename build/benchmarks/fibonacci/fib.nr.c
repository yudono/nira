...
bool is_truthy_unused(Value v) { if (v.type == VAL_NIL) return false; if (v.type == VAL_BOOL || v.type == VAL_INT) return v.data.i != 0; if (v.type == VAL_FLOAT) return v.data.f != 0.0; return true; }
...
Value nr_rt_push(Value arr, Value val) { if (arr.type != VAL_ARR) return val_nil(); if (arr.data.arr->count >= arr.data.arr->capacity) { int old_cap = arr.data.arr->capacity; arr.data.arr->capacity *= 2; Value* new_el = nr_alloc(sizeof(Value) * arr.data.arr->capacity); memcpy(new_el, arr.data.arr->elements, sizeof(Value)*old_cap); arr.data.arr->elements = new_el; }  arr.data.arr->elements[arr.data.arr->count] = val; arr.data.arr->count++; return val; }
...
Value nr_fib(Value self, Value nr_v_a1, Value nr_v_a2, Value nr_v_a3, Value nr_v_a4, Value nr_v_a5);
Value nr_v_time;
Value nr_v_millis;
Value nr_v_time_now;
...
Value nr_fib(Value self, Value nr_v_a1, Value nr_v_a2, Value nr_v_a3, Value nr_v_a4, Value nr_v_a5) {
  Value nr_v_n = nr_v_a1;
  if (nr_rt_lt_bool(nr_v_n, val_int(2))) {
  return nr_v_n;
  };
  return nr_rt_add(nr_fib(val_nil(), val_int(nr_v_n.data.i - val_int(1).data.i), val_nil(), val_nil(), val_nil(), val_nil()), nr_fib(val_nil(), val_int(nr_v_n.data.i - val_int(2).data.i), val_nil(), val_nil(), val_nil(), val_nil()));
...
;
  ...
  nr_v_result = nr_fib(val_nil(), val_int(40), val_nil(), val_nil(), val_nil(), val_nil());
...
nr_rt_print(nr_rt_add(nr_rt_add(nr_rt_add(nr_rt_add(val_str("Nira: "), nr_rt_to_string(val_int(nr_v_end.data.i - nr_v_start.data.i))), val_str(" ms (Result: ")), nr_rt_to_string(nr_v_result)), val_str(")")));
