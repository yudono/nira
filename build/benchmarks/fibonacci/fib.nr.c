...
Value nr_fib(Value self, Value _v0, Value _v1, Value _v2, Value _v3, Value _v4);
...
Value nr_v_fib;
static long long _fast_fib(long long n) { if (n < 2) return n; return _fast_fib(n - 1) + _fast_fib(n - 2); }
Value nr_fib(Value self, Value _v0, Value _v1, Value _v2, Value _v3, Value _v4) { return val_int(_fast_fib(_v0.data.i)); }
int main(int argc, char** argv) {
  ...
  long long nr_v_start = 0;
  long long nr_v_end = 0;
  long long nr_v_sum = 0;
  long long nr_v_i = 0;
  long long nr_v_j = 0;
  long long nr_v_n = 0;
  long long nr_v_temp = 0;
  nr_v_s = malloc(10 * 1024 * 1024); nr_v_s_len = 0;
  long long nr_v_result = 0;
  long long nr_v_millis = 0;
  nr_v_arr_unboxed = malloc(sizeof(long long) * 100000); nr_v_arr_count = 0; nr_v_arr = val_arr();
  long long nr_v_count = 0;
  Value nr_v_fib = val_nil();
;
;
;
  nr_v_start = (nr_rt_millis()).data.i;
  nr_v_result = (val_int(_fast_fib((val_int(40)).data.i))).data.i;
  nr_v_end = (nr_rt_millis()).data.i;
nr_rt_print(nr_rt_add(nr_rt_add(nr_rt_add(nr_rt_add(val_str("Nira: "), nr_rt_to_string(val_int((nr_v_end - nr_v_start)))), val_str(" ms (Result: ")), nr_rt_to_string(val_int(nr_v_result))), val_str(")")));
  return 0; 
}
