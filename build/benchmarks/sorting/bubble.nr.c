...
;
  ...
  nr_v_i = val_int(0);
  while (is_truthy(val_bool(nr_v_i.data.i < val_int(1000).data.i))) {
({ Value _t = nr_v_arr; Value _f = get_field(_t, "push"); Value _r; if (_f.type == VAL_FUNC) _r = ((Value (*)(Value, Value, Value, Value, Value, Value))_f.data.func_ptr)(_t, val_int(val_int(1000).data.i - nr_v_i.data.i), val_nil(), val_nil(), val_nil(), val_nil()); else { _r = nr_rt_push(_t, val_int(val_int(1000).data.i - nr_v_i.data.i)); } _r; });
...
;
  ...
  nr_v_i = val_int(0);
  while (is_truthy(val_bool(nr_v_i.data.i < nr_v_n.data.i))) {
  nr_v_j = val_int(0);
  while (is_truthy(val_bool(nr_v_j.data.i < val_int(val_int(nr_v_n.data.i - nr_v_i.data.i).data.i - val_int(1).data.i).data.i))) {
  if (is_truthy(val_bool(nr_rt_at(nr_v_arr, nr_v_j).data.i > nr_rt_at(nr_v_arr, nr_rt_add(nr_v_j, val_int(1))).data.i))) {
...
nr_rt_print(nr_rt_add(nr_rt_add(nr_rt_add(nr_rt_add(nr_rt_add(nr_rt_add(val_str("Nira: "), nr_rt_to_string(val_int(nr_v_end.data.i - nr_v_start.data.i))), val_str(" ms (First: ")), nr_rt_to_string(nr_rt_at(nr_v_arr, val_int(0)))), val_str(", Last: ")), nr_rt_to_string(nr_rt_at(nr_v_arr, val_int(999)))), val_str(")")));
