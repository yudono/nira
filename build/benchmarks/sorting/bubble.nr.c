...
;
  ...
  nr_v_i = 0LL;
  while ((nr_v_i < 5000LL)) {
({ nr_v_arr_unboxed[nr_v_arr_count++] = ((5000LL - nr_v_i)); val_nil(); });
  nr_v_i = (nr_v_i + 1LL);
  }
;
  nr_v_start = (nr_rt_millis()).data.i;
  nr_v_n = (val_int(nr_v_arr_count)).data.i;
  nr_v_i = 0LL;
  while ((nr_v_i < nr_v_n)) {
  nr_v_j = 0LL;
  while ((nr_v_j < ((nr_v_n - nr_v_i) - 1LL))) {
...
nr_rt_print(nr_rt_add(nr_rt_add(nr_rt_add(nr_rt_add(nr_rt_add(nr_rt_add(val_str("Nira: "), nr_rt_to_string(val_int((nr_v_end - nr_v_start)))), val_str(" ms (First: ")), nr_rt_to_string(val_int(nr_v_arr_unboxed[0LL]))), val_str(", Last: ")), nr_rt_to_string(val_int(nr_v_arr_unboxed[4999LL]))), val_str(")")));
  return 0; 
}
