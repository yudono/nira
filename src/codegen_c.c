...
static char *extra_headers[64];
static int extra_header_count = 0;
static char *extra_links[64];
static int extra_link_count = 0;
static char *emitted_native_codes[64];
static int emitted_native_code_count = 0;
...
static char *collected_funcs_modules[128];
static int collected_funcs_module_count = 0;
...
static void print_runtime(FILE *out) {
  ...
  fprintf(
      out,
      "Value val_obj() { Value v; v.type = VAL_OBJ; v.data.obj = "
      "nr_alloc(sizeof(*v.data.obj)); v.data.obj->count = 0; "
      "v.data.obj->capacity = 8; v.data.obj->keys = nr_alloc(sizeof(char*) * "
      "8); v.data.obj->values = nr_alloc(sizeof(Value*) * 8); return v; }\n");
  fprintf(out, "Value val_arr() { Value v; v.type = VAL_ARR; v.data.arr = "
               "nr_alloc(sizeof(*v.data.arr)); v.data.arr->count = 0; "
               "v.data.arr->capacity = 8; v.data.arr->elements = "
               "nr_alloc(sizeof(Value*) * 8); return v; }\n\n");
  fprintf(
      out,
      "void set_field(Value obj, const char* key, Value val) { if (obj.type != "
      "VAL_OBJ) return; for(int i=0; i<obj.data.obj->count; i++) { "
      "if(strcmp(obj.data.obj->keys[i], key) == 0) { *obj.data.obj->values[i] "
      "= val; return; } } if(obj.data.obj->count >= obj.data.obj->capacity) { "
      "int old_cap = obj.data.obj->capacity; obj.data.obj->capacity *= 2; "
      ...
      "obj.data.obj->keys = new_keys; obj.data.obj->values = new_vals; } "
      ...
      "obj.data.obj->count++; }\n");
  fprintf(out, "Value get_field(Value obj, const char* key) { if (obj.type != "
               "VAL_OBJ) return val_nil(); for(int i=0; i<obj.data.obj->count; "
               "i++) { if(strcmp(obj.data.obj->keys[i], key) == 0) return "
               "*obj.data.obj->values[i]; } return val_nil(); }\n\n");
  ...
  fprintf(out,
          ...
          "val_arr(); Value a = val_arr(); for(int i=0; i<obj.data.obj->count; "
          "i++) nr_rt_push(a, val_str(obj.data.obj->keys[i])); return a; }\n");
  fprintf(out,
          "Value nr_rt_pop(Value arr) { if (arr.type != VAL_ARR || "
          "arr.data.arr->count == 0) return val_nil(); arr.data.arr->count--; "
          "return *arr.data.arr->elements[arr.data.arr->count]; }\n");
  fprintf(
      out,
  ...
  fprintf(out, "Value nr_rt_to_int(Value v) { if (v.type == VAL_INT) return v; "
               "if (v.type == VAL_STR) return val_int((int)strtol(v.data.s, "
  ...
  fprintf(
      ...
      "(v.type == VAL_OBJ) printf(\"[Object]\\n\"); else if (v.type == "
      "VAL_ERROR) printf(\"Error: %%s\\n\", v.data.s); else "
      "printf(\"nil\\n\"); fflush(stdout); }\n");
  fprintf(out, "Value nr_rt_len(Value v) { if (v.type == VAL_ARR) return "
  ...
  fprintf(out, "    if (l.type == VAL_STR) return val_bool(strcmp(l.data.s, "
               "r.data.s) == 0);\n");
  fprintf(out, "    if (l.type == VAL_INT || l.type == VAL_BOOL) return "
               "val_bool(l.data.i == r.data.i);\n");
  fprintf(
      out,
      "    if (l.type == VAL_FLOAT) return val_bool(l.data.f == r.data.f);\n");
  fprintf(out, "    return val_bool(l.data.obj == r.data.obj);\n");
  fprintf(out, "  }\n");
  fprintf(out, "  if ((l.type == VAL_INT || l.type == VAL_FLOAT) && (r.type == "
  ...
  fprintf(out, "Value nr_rt_read_file(Value path) { if (path.type != VAL_STR) "
               ...
               "ftell(f); rewind(f); char* b = nr_alloc(sz + 1); fread(b, 1, "
               "sz, f); b[sz] = 0; fclose(f); return val_str(b); }\n");
  fprintf(
      out,
      "Value nr_rt_file_write(Value path, Value content) { if (path.type != "
      "VAL_STR || content.type != VAL_STR) return val_nil(); if "
      "(!is_safe_path(path.data.s)) return val_error(\"Path traversal "
      "detected\"); FILE* f = fopen(path.data.s, \"w\"); if (!f) return "
      "val_nil(); fputs(content.data.s, f); fclose(f); return val_int(1); }\n");
  fprintf(out, "Value nr_rt_file_exists(Value path) { if (path.type != "
               "VAL_STR) return val_bool(false); if "
               "(!is_safe_path(path.data.s)) return val_bool(false); struct "
               "stat st; return val_bool(stat(path.data.s, &st) == 0); }\n");
  fprintf(
      out,
  ...
  fprintf(
      ...
      "VAL_STR || start.type != VAL_INT || len.type != VAL_INT) return "
      "val_nil(); int slen = strlen(s.data.s); int istart = start.data.i; int "
      "ilen = len.data.i; if (istart < 0) istart = 0; if (istart >= slen) "
      "return val_str(\"\"); if (ilen < 0) ilen = 0; if (istart + ilen > slen) "
      "ilen = slen - istart; char* sub = nr_alloc(ilen + 1); strncpy(sub, "
      "s.data.s + istart, ilen); sub[ilen] = 0; return val_str(sub); }\n\n");
  fprintf(out, "Value nr_rt_exec(Value cmd) { if (cmd.type != VAL_STR) return "
               "val_int(-1); for(char* p=cmd.data.s; *p; p++) { "
               "if(!isalnum(*p) && *p!='.' && *p!='/' && *p!='_' && *p!='-' && "
               "*p!=' ') return val_error(\"Illegal character in command\"); } "
               "return val_int(system(cmd.data.s)); }\n");
  fprintf(out, "Value nr_rt_str_index_of(Value s, Value sub) { if (s.type != "
               "VAL_STR || sub.type != VAL_STR) return val_int(-1); char* p = "
               "strstr(s.data.s, sub.data.s); if (!p) return val_int(-1); int "
               "res = (int)(p - s.data.s); return val_int(res); }\n");
  fprintf(out,
          "Value nr_rt_str_match(Value s, Value regex_str) { if (s.type != "
          "VAL_STR || regex_str.type != VAL_STR) return val_bool(0); regex_t "
          "regex; int reti = regcomp(&regex, regex_str.data.s, REG_EXTENDED); "
          "if (reti) return val_bool(0); reti = regexec(&regex, s.data.s, 0, "
          "NULL, 0); regfree(&regex); return val_bool(!reti); }\n");
  fprintf(out,
  ...
  fprintf(out, "Value nr_rt_to_string(Value v) { char buf[64]; if (v.type == "
               ...
               "strcpy(buf, \"null\"); else strcpy(buf, \"[object]\"); return "
               "val_str(nr_strdup(buf)); }\n");
  fprintf(out, "void nr_rt_exit(Value code) { exit(code.type == VAL_INT ? "
               "code.data.i : 0); }\n");
  fprintf(out,
          "Value nr_rt_str_replace(Value s, Value old, Value new_str) {\n");
  fprintf(out, "  if (s.type != VAL_STR || old.type != VAL_STR || new_str.type "
               "!= VAL_STR) return s;\n");
  fprintf(out, "  char *res; int i, count = 0; int oldlen = "
               "strlen(old.data.s); int newlen = strlen(new_str.data.s);\n");
  fprintf(out,
          "  for (i = 0; s.data.s[i] != '\\0'; i++) { if (strstr(&s.data.s[i], "
          "old.data.s) == &s.data.s[i]) { count++; i += oldlen - 1; } }\n");
  fprintf(out,
          "  res = (char *)nr_alloc(i + count * (newlen - oldlen) + 1);\n");
  fprintf(out, "  i = 0; while (*s.data.s) { if (strstr(s.data.s, old.data.s) "
               "== s.data.s) { strcpy(&res[i], new_str.data.s); i += newlen; "
               "s.data.s += oldlen; } else res[i++] = *s.data.s++; } res[i] = "
               "'\\0'; return val_str(res);\n");
  fprintf(out, "}\n");
  ...
  fprintf(
      ...
      "char* res = nr_strdup(\"[\"); for (int i=0; i<v.data.arr->count; i++) { "
      "char* item = val_to_json(*v.data.arr->elements[i]); char* old = res; "
      "res = nr_alloc(strlen(old) + strlen(item) + 3); sprintf(res, "
      "\"%%s%%s%%s\", old, item, i < v.data.arr->count-1 ? \",\" : \"\"); } "
      "char* old = res; res = nr_alloc(strlen(old) + 2); sprintf(res, "
      "\"%%s]\", old); return res; } if (v.type == VAL_OBJ) { char* res = "
      "nr_strdup(\"{\"); for (int i=0; i<v.data.obj->count; i++) { char* val = "
      "val_to_json(*v.data.obj->values[i]); char* old = res; res = "
      "nr_alloc(strlen(old) + strlen(v.data.obj->keys[i]) + strlen(val) + 6); "
  ...
  fprintf(out, "  char* p = s.data.s; while(*p == ' ' || *p == '[' || *p == "
               "']') p++;\n");
  fprintf(out, "  if (*p == '{') {\n");
  fprintf(out, "    Value obj = val_obj(); p++;\n");
  fprintf(out, "    while(*p && *p != '}') {\n");
  fprintf(out, "      while(*p == ' ' || *p == '\"' || *p == ',') p++;\n");
  fprintf(out, "      char key[64]; int i=0; while(*p && *p != '\"') key[i++] "
               "= *p++; key[i] = 0; p++;\n");
  fprintf(out, "      while(*p == ' ' || *p == ':') p++;\n");
  fprintf(out, "      if (*p == '\"') { p++; char val[256]; int j=0; while(*p "
               "&& *p != '\"') val[j++] = *p++; val[j] = 0; p++; "
               "set_field(obj, key, val_str(val)); }\n");
  fprintf(out, "      else { char val[32]; int j=0; while(*p && *p != ',' && "
               "*p != ' ' && *p != '}') val[j++] = *p++; val[j] = 0; "
               "set_field(obj, key, val_int(atoi(val))); }\n");
  fprintf(out, "    } return obj;\n");
  fprintf(out, "  } return val_nil();\n");
  fprintf(out, "}\n");
  fprintf(out, "Value nr_rt_http_serve(Value port, Value callback) {\n");
  fprintf(out, "  #ifdef _WIN32\n  WSADATA wsaData; WSAStartup(MAKEWORD(2,2), "
               "&wsaData);\n  #endif\n");
  fprintf(out, "  int server_fd, new_socket; struct sockaddr_in address; int "
               "addrlen = sizeof(address);\n");
  fprintf(out, "  server_fd = socket(AF_INET, SOCK_STREAM, 0);\n");
  fprintf(out, "  int opt = 1; setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR, "
               "(char*)&opt, sizeof(opt));\n");
  fprintf(out, "  address.sin_family = AF_INET; address.sin_addr.s_addr = "
               "INADDR_ANY; address.sin_port = htons(port.data.i);\n");
  fprintf(out,
          "  if (bind(server_fd, (struct sockaddr *)&address, sizeof(address)) "
          "< 0) { perror(\"bind\"); close(server_fd); return val_nil(); }\n");
  fprintf(out, "  if (listen(server_fd, 3) < 0) { perror(\"listen\"); "
  ...
  fprintf(out, "  while(1) {\n    void* cp = nr_checkpoint();\n    new_socket "
               ...
               "(socklen_t*)&addrlen);\n");
  fprintf(out, "    char buffer[30000] = {0}; int n = recv(new_socket, buffer, "
               "30000, 0);\n");
  fprintf(
      out,
      "    if (n <= 0) { close(new_socket); nr_rollback(cp); continue; }\n");
  fprintf(out, "    char method[16]={0}, path[1024]={0};\n");
  fprintf(out, "    if(sscanf(buffer, \"%%15s %%1023s\", method, path) < 2) { "
               "close(new_socket); nr_rollback(cp); continue; }\n");
  fprintf(out, "    Value req = val_obj(); set_field(req, \"method\", "
               "val_str(method)); set_field(req, \"path\", val_str(path));\n");
  fprintf(
      out,
      "    char* body_ptr = strstr(buffer, \"\\r\\n\\r\\n\"); if (body_ptr) { "
      "body_ptr += 4; set_field(req, \"body\", val_str(body_ptr)); } else { "
      "set_field(req, \"body\", val_nil()); }\n");
  fprintf(out, "    Value res = ((Value (*)(Value, Value, Value, Value, Value, "
               "Value))callback.data.func_ptr)(val_nil(), req, val_nil(), "
               "val_nil(), val_nil(), val_nil());\n");
  fprintf(out, "    char* res_body = \"Not Found\"; int status = 404;\n");
  fprintf(out, "    if (res.type == VAL_OBJ) {\n");
  fprintf(out, "      Value b = get_field(res, \"body\"); if (b.type == "
               "VAL_STR) res_body = b.data.s;\n");
  fprintf(out, "      Value s = get_field(res, \"status\"); if (s.type == "
               "VAL_INT) status = s.data.i;\n");
  fprintf(out, "    }\n");
  fprintf(out, "    char response[32000];\n");
  fprintf(out, "    snprintf(response, sizeof(response), \"HTTP/1.1 %%d "
               "OK\\r\\nContent-Type: application/json\\r\\nContent-Length: "
               "%%ld\\r\\n\\r\\n%%s\", status, strlen(res_body), res_body);\n");
  fprintf(out, "    send(new_socket, response, strlen(response), 0); "
               "close(new_socket);\n");
...
static AstNode *nr_load_module(const char *m) {
  ...
  for (int i = 0; i < include_path_count; i++) {
    sprintf(path, "%s/%s.nr", include_paths[i], m);
  ...
  if (!src) {
    sprintf(path, ".nira/libs/%s/%s.nr", m, m);
    src = nr_read_file(path);
  }
  if (!src) {
...
static void pre_collect_native(AstNode *node) {
  if (!node)
    return;
  switch (node->type) {
  case AST_PROGRAM:
    for (int i = 0; i < node->data.program.count; i++)
      pre_collect_native(node->data.program.statements[i]);
    break;
  case AST_IMPORT: {
    AstNode *prog = nr_load_module(node->data.import_stmt.path);
    node->data.import_stmt.module_prog = prog;
    if (prog)
      pre_collect_native(prog);
    break;
  }
  case AST_FUNC_DECL:
    pre_collect_native(node->data.func_decl.body);
    break;
  case AST_IF:
    pre_collect_native(node->data.if_stmt.then_branch);
    if (node->data.if_stmt.else_branch)
      pre_collect_native(node->data.if_stmt.else_branch);
    break;
  case AST_WHILE:
    pre_collect_native(node->data.while_stmt.body);
    break;
  case AST_FOR:
    pre_collect_native(node->data.for_stmt.body);
    break;
  case AST_NATIVE: {
    for (int i = 0; i < node->data.native_stmt.link_count; i++) {
      int dup = 0;
      for (int j = 0; j < extra_link_count; j++)
        if (strcmp(extra_links[j], node->data.native_stmt.links[i]) == 0) {
          dup = 1;
          break;
        }
      if (!dup && extra_link_count < 64)
        extra_links[extra_link_count++] =
            strdup(node->data.native_stmt.links[i]);
    }
    for (int i = 0; i < node->data.native_stmt.header_count; i++) {
      int dup = 0;
      for (int j = 0; j < extra_header_count; j++)
        if (strcmp(extra_headers[j], node->data.native_stmt.headers[i]) == 0) {
          dup = 1;
          break;
        }
      if (!dup && extra_header_count < 64)
        extra_headers[extra_header_count++] =
            strdup(node->data.native_stmt.headers[i]);
    }
    break;
  }
  case AST_EXTERN: {
...
static void collect_functions(AstNode *node, FILE *out) {
  ...
  case AST_PROGRAM:
    for (int i = 0; i < node->data.program.count; i++)
      collect_functions(node->data.program.statements[i], out);
    break;
  case AST_FUNC_DECL: {
    ...
    if (strcmp(node->data.func_decl.name, "main") != 0) {
      fprintf(out,
              "Value nr_%s(Value self, Value nr_v_a1, Value nr_v_a2, Value "
              "nr_v_a3, Value nr_v_a4, Value nr_v_a5);\n",
  ...
  case AST_IMPORT: {
    ...
    for (int i = 0; i < collected_funcs_module_count; i++)
      if (strcmp(collected_funcs_modules[i], m) == 0)
        return;
    if (collected_funcs_module_count < 128)
      collected_funcs_modules[collected_funcs_module_count++] = strdup(m);

    AstNode *prog = nr_load_module(m);
    node->data.import_stmt.module_prog = prog;
    if (prog) {
      fprintf(out, "void nr_%s_init();\n", m);
      char *old = current_mod;
      current_mod = (char *)m;
      collect_functions(prog, out);
      current_mod = old;
    }
    break;
  }
  case AST_IF:
    collect_functions(node->data.if_stmt.condition, out);
    collect_functions(node->data.if_stmt.then_branch, out);
    if (node->data.if_stmt.else_branch)
      collect_functions(node->data.if_stmt.else_branch, out);
    break;
  case AST_WHILE:
    collect_functions(node->data.while_stmt.condition, out);
    collect_functions(node->data.while_stmt.body, out);
    break;
  case AST_FOR:
    collect_functions(node->data.for_stmt.iterable, out);
  ...
  case AST_CALL:
    for (int i = 0; i < node->data.call.arg_count; i++)
      collect_functions(node->data.call.args[i], out);
    break;
  case AST_BINARY:
    collect_functions(node->data.binary.left, out);
    collect_functions(node->data.binary.right, out);
    break;
  case AST_ARRAY:
    for (int i = 0; i < node->data.array.count; i++)
      collect_functions(node->data.array.elements[i], out);
    break;
  case AST_OBJECT:
    for (AstField *f = node->data.object.fields; f; f = f->next)
      collect_functions(f->value, out);
    break;
  case AST_DESTRUCTURING:
    collect_functions(node->data.destruct.value, out);
    break;
  case AST_NATIVE: {
    if (node->data.native_stmt.code) {
      int dup = 0;
      for (int i = 0; i < emitted_native_code_count; i++) {
        if (strcmp(emitted_native_codes[i], node->data.native_stmt.code) == 0) {
          dup = 1;
          break;
        }
      }
      if (!dup && emitted_native_code_count < 64) {
        emitted_native_codes[emitted_native_code_count++] =
            strdup(node->data.native_stmt.code);
...
static void collect_vars(AstNode *node, const char **declared, int *count) {
  ...
  if (node->type == AST_ASSIGN && !strchr(node->data.assign.target, '.')) {
    for (int i = 0; i < *count; i++)
      if (strcmp(declared[i], node->data.assign.target) == 0)
        goto next;
    if (*count < 1024)
      declared[(*count)++] = strdup(node->data.assign.target);
  }
next:;
  switch (node->type) {
  case AST_PROGRAM:
    for (int i = 0; i < node->data.program.count; i++)
      collect_vars(node->data.program.statements[i], declared, count);
    break;
  case AST_IF:
    collect_vars(node->data.if_stmt.then_branch, declared, count);
    if (node->data.if_stmt.else_branch)
      collect_vars(node->data.if_stmt.else_branch, declared, count);
    break;
  case AST_FOR: {
    if (*count < 1024)
      declared[(*count)++] = strdup(node->data.for_stmt.var);
    if (node->data.for_stmt.alias && *count < 1024)
      declared[(*count)++] = strdup(node->data.for_stmt.alias);
    collect_vars(node->data.for_stmt.body, declared, count);
    break;
  }
  case AST_WHILE:
    collect_vars(node->data.while_stmt.body, declared, count);
    break;
  case AST_IMPORT: {
    const char *target = node->data.import_stmt.alias
                             ? node->data.import_stmt.alias
                             : node->data.import_stmt.path;
    if (*count < 1024)
      declared[(*count)++] = strdup(target);
    for (int i = 0; i < node->data.import_stmt.symbol_count; i++)
      if (*count < 1024)
        declared[(*count)++] = strdup(node->data.import_stmt.symbols[i]);
    break;
  }
  case AST_DESTRUCTURING: {
    AstNode *t = node->data.destruct.target;
    if (t->type == AST_OBJECT) {
      for (AstField *f = t->data.object.fields; f; f = f->next)
        if (*count < 1024)
          declared[(*count)++] = strdup(f->alias ? f->alias : f->name);
    }
    break;
  }
  case AST_FUNC_DECL: {
    if (node->data.func_decl.name &&
        ...
        strcmp(node->data.func_decl.name, "anonymous") != 0) {
      if (*count < 1024)
        declared[(*count)++] = strdup(node->data.func_decl.name);
    }
    break;
  }
  default:
...
static void generate_functions(AstNode *node, AstNode *mod, FILE *out) {
  ...
  case AST_PROGRAM:
    for (int i = 0; i < node->data.program.count; i++)
      generate_functions(node->data.program.statements[i], node, out);
    break;
  case AST_IMPORT: {
    const char *m = node->data.import_stmt.path;
    for (int i = 0; i < generated_module_count; i++) {
      if (strcmp(generated_modules[i], m) == 0) {
        printf("  Skip module %s (already in generated_modules[%d]: %s)\n", m, i, generated_modules[i]);
        return;
      }
    }
    if (generated_module_count < 128)
      generated_modules[generated_module_count++] = strdup(m);
    
    if (node->data.import_stmt.module_prog) {
        AstNode* p = node->data.import_stmt.module_prog;
        generate_functions(p, p, out);
      fprintf(out,
              "void nr_%s_init() { static bool init = false; if (init) return; "
              "init = true;\n",
              m);
      fprintf(out, "  if (nr_v_%s.type == VAL_NIL) nr_v_%s = val_obj();\n", m,
              m);
      for (int i = 0;
           i < node->data.import_stmt.module_prog->data.program.count; i++) {
        AstNode *s =
            node->data.import_stmt.module_prog->data.program.statements[i];
        if (s->type == AST_FUNC_DECL) {
          const char *n = s->data.func_decl.name;
          if (strcmp(n, "main") != 0 && strcmp(n, "anonymous") != 0) {
            const char *sn = strrchr(n, '_');
            if (sn)
              sn++;
            else
              sn = n;
  ...
  case AST_FUNC_DECL: {
    ...
    char *last_und = strrchr(name, '_');
    char *old_pref = current_pref;
    if (strncmp(name, "lambda", 6) != 0) {
      if (last_und) {
        char m_name[64];
        int len = last_und - name;
        strncpy(m_name, name, len);
        m_name[len] = 0;
        current_pref = strdup(m_name);
      } else
        current_pref = NULL;
    }
    ...
    for (int i = 0; i < vcount; i++) {
      int skip = 0;
      if (strcmp(vars[i], "self") == 0)
        skip = 1;
      for (int j = 0; j < node->data.func_decl.param_count; j++)
        if (strcmp(vars[i], node->data.func_decl.params[j]) == 0)
          skip = 1;
      if (is_global(vars[i]))
        skip = 1;
      if (!skip) {
        int dup = 0;
        for (int k = 0; k < i; k++)
          if (strcmp(vars[k], vars[i]) == 0) {
            dup = 1;
            break;
          }
        if (!dup)
          fprintf(out, "  Value nr_v_%s = val_nil(); (void)nr_v_%s;\n", vars[i],
                  vars[i]);
      }
    }
    if (node->data.func_decl.body->type != AST_PROGRAM) {
      fprintf(out, "  return ");
  ...
  case AST_IF:
    generate_functions(node->data.if_stmt.then_branch, mod, out);
    if (node->data.if_stmt.else_branch)
      generate_functions(node->data.if_stmt.else_branch, mod, out);
    break;
  case AST_WHILE:
    generate_functions(node->data.while_stmt.body, mod, out);
    break;
  case AST_FOR:
    generate_functions(node->data.for_stmt.body, mod, out);
    break;
  case AST_ASSIGN:
    generate_functions(node->data.assign.value, mod, out);
    break;
  case AST_CALL:
    for (int i = 0; i < node->data.call.arg_count; i++)
      generate_functions(node->data.call.args[i], mod, out);
    break;
  case AST_ARRAY:
    for (int i = 0; i < node->data.array.count; i++)
      generate_functions(node->data.array.elements[i], mod, out);
    break;
  case AST_OBJECT:
    for (AstField *f = node->data.object.fields; f; f = f->next)
      generate_functions(f->value, mod, out);
    break;
  case AST_DESTRUCTURING:
    generate_functions(node->data.destruct.value, mod, out);
    break;
  case AST_RETURN:
    generate_functions(node->data.ret.value, mod, out);
    break;
  case AST_BINARY:
    generate_functions(node->data.binary.left, mod, out);
    generate_functions(node->data.binary.right, mod, out);
    break;
  default:
    break;
  }
...
void codegen_c_node(AstNode *node, FILE *out) {
  ...
  case AST_PROGRAM:
    for (int i = 0; i < node->data.program.count; i++) {
      codegen_c_node(node->data.program.statements[i], out);
      fprintf(out, ";\n");
    }
    break;
  case AST_RETURN:
  ...
  case AST_FOR: {
    ...
    fprintf(out, ".type == VAL_ARR) {\n");
    char buf[32];
    sprintf(buf, "_i%d", ++global_lambda_count);
    ...
    if (node->data.for_stmt.alias)
      fprintf(out, "    nr_v_%s = nr_v_%s;\n", node->data.for_stmt.alias,
  ...
  case AST_DESTRUCTURING: {
    ...
    while (f) {
      const char *n = f->alias ? f->alias : f->name;
      fprintf(out, "nr_v_%s = get_field(_tmp, \"%s\"); ", n, f->name);
      f = f->next;
    }
    fprintf(out, "_tmp; })");
    break;
  ...
  case AST_CALL: {
    ...
    if (strcmp(n, "print") == 0) {
      ...
      fprintf(out, ")");
    } else if (strcmp(n, "len") == 0 || strcmp(n, "__builtin_len") == 0) {
      fprintf(out, "nr_rt_len(");
      if (node->data.call.arg_count > 0)
        codegen_c_node(node->data.call.args[0], out);
      fprintf(out, ")");
    } else if (strcmp(n, "__builtin_push") == 0) {
      fprintf(out, "nr_rt_push(");
      if (node->data.call.arg_count > 0)
        codegen_c_node(node->data.call.args[0], out);
      if (node->data.call.arg_count > 1) {
        fprintf(out, ", ");
        codegen_c_node(node->data.call.args[1], out);
      }
      fprintf(out, ")");
    } else if (strcmp(n, "__builtin_pop") == 0) {
      fprintf(out, "nr_rt_pop(");
      if (node->data.call.arg_count > 0)
        codegen_c_node(node->data.call.args[0], out);
      fprintf(out, ")");
    } else if (strcmp(n, "__builtin_file_read") == 0) {
      fprintf(out, "nr_rt_file_read(");
      if (node->data.call.arg_count > 0)
        codegen_c_node(node->data.call.args[0], out);
      fprintf(out, ")");
    } else if (strcmp(n, "__builtin_file_write") == 0) {
      fprintf(out, "nr_rt_file_write(");
      if (node->data.call.arg_count > 0)
        codegen_c_node(node->data.call.args[0], out);
      if (node->data.call.arg_count > 1) {
        fprintf(out, ", ");
      ...
      fprintf(out, ")");
    } else if (strcmp(n, "__builtin_file_exists") == 0) {
      fprintf(out, "nr_rt_file_exists(");
      if (node->data.call.arg_count > 0)
        codegen_c_node(node->data.call.args[0], out);
      fprintf(out, ")");
    } else if (strcmp(n, "__builtin_file_delete") == 0) {
      fprintf(out, "nr_rt_file_delete(");
      if (node->data.call.arg_count > 0)
        codegen_c_node(node->data.call.args[0], out);
      fprintf(out, ")");
    } else if (strcmp(n, "__builtin_now") == 0 ||
               strcmp(n, "__builtin_time_now") == 0)
      fprintf(out, "nr_rt_now()");
    else if (strcmp(n, "__builtin_sleep") == 0 ||
             strcmp(n, "__builtin_delay") == 0) {
      fprintf(out, "nr_rt_sleep(");
      if (node->data.call.arg_count > 0)
        codegen_c_node(node->data.call.args[0], out);
      fprintf(out, ")");
    } else if (strcmp(n, "__builtin_random") == 0)
      fprintf(out, "nr_rt_random()");
    else if (strcmp(n, "__builtin_substring") == 0) {
      fprintf(out, "nr_rt_substring(");
      for (int i = 0; i < node->data.call.arg_count; i++) {
        codegen_c_node(node->data.call.args[i], out);
        if (i < node->data.call.arg_count - 1)
          fprintf(out, ", ");
      ...
      fprintf(out, ")");
    } else if (strcmp(n, "__builtin_exec") == 0) {
      fprintf(out, "nr_rt_exec(");
      if (node->data.call.arg_count > 0)
        codegen_c_node(node->data.call.args[0], out);
      fprintf(out, ")");
    } else if (strcmp(n, "__builtin_exit_proc") == 0) {
      fprintf(out, "nr_rt_exit(");
      if (node->data.call.arg_count > 0)
        codegen_c_node(node->data.call.args[0], out);
      fprintf(out, ")");
    } else if (strcmp(n, "toInt") == 0) {
      fprintf(out, "nr_rt_to_int(");
      if (node->data.call.arg_count > 0)
        codegen_c_node(node->data.call.args[0], out);
      fprintf(out, ")");
      ...
      fprintf(out, "nr_rt_http_serve(");
      codegen_c_node(node->data.call.args[0], out);
      fprintf(out, ", ");
      codegen_c_node(node->data.call.args[1], out);
      fprintf(out, ")");
    } else if (strncmp(n, "__native_", 9) == 0) {
      fprintf(out, "%s(", n);
      for (int i = 0; i < node->data.call.arg_count; i++) {
        codegen_c_node(node->data.call.args[i], out);
        if (i < node->data.call.arg_count - 1)
      ...
      fprintf(out, ")");
    } else if (strcmp(n, "json_decode") == 0) {
      fprintf(out, "nr_rt_json_decode(");
      if (node->data.call.arg_count > 0)
        codegen_c_node(node->data.call.args[0], out);
      fprintf(out, ")");
    } else if (strcmp(n, "__builtin_json_encode") == 0) {
      fprintf(out, "nr_rt_json_encode(");
      codegen_c_node(node->data.call.args[0], out);
      fprintf(out, ")");
    } else if (strcmp(n, "__builtin_str_index_of") == 0) {
      fprintf(out, "nr_rt_str_index_of(");
      codegen_c_node(node->data.call.args[0], out);
      fprintf(out, ", ");
      codegen_c_node(node->data.call.args[1], out);
      fprintf(out, ")");
    } else if (strcmp(n, "__builtin_str_replace") == 0) {
      fprintf(out, "nr_rt_str_replace(");
      codegen_c_node(node->data.call.args[0], out);
      fprintf(out, ", ");
      codegen_c_node(node->data.call.args[1], out);
      fprintf(out, ", ");
      codegen_c_node(node->data.call.args[2], out);
      fprintf(out, ")");
    } else if (strcmp(n, "__builtin_str_match") == 0) {
      fprintf(out, "nr_rt_str_match(");
      codegen_c_node(node->data.call.args[0], out);
      fprintf(out, ", ");
      codegen_c_node(node->data.call.args[1], out);
      fprintf(out, ")");
    } else if (strcmp(n, "__builtin_obj_keys") == 0) {
      fprintf(out, "nr_rt_obj_keys(");
      codegen_c_node(node->data.call.args[0], out);
      fprintf(out, ")");
    } else if (strchr(n, '.')) {
      ...
      char *d = strchr(t, '.');
      *d = 0;
      const char *method = d + 1;
      fprintf(
          out,
          "({ Value _t = nr_v_%s; Value _f = get_field(_t, \"%s\"); Value _r; ",
          t, method);
      fprintf(out, "if (_f.type == VAL_FUNC) _r = ((Value (*)(Value, Value, "
                   "Value, Value, Value, Value))_f.data.func_ptr)(_t");
      for (int i = 0; i < node->data.call.arg_count; i++) {
        fprintf(out, ", ");
        codegen_c_node(node->data.call.args[i], out);
      }
      for (int i = node->data.call.arg_count; i < 5; i++)
        fprintf(out, ", val_nil()");
      fprintf(out, "); else { ");
      if (strcmp(method, "substring") == 0) {
        if (node->data.call.arg_count == 3) {
          fprintf(out, "_r = nr_rt_substring(");
          codegen_c_node(node->data.call.args[0], out);
          fprintf(out, ", ");
          codegen_c_node(node->data.call.args[1], out);
          fprintf(out, ", ");
          ...
          fprintf(out, "_r = nr_rt_substring(_t");
          for (int i = 0; i < node->data.call.arg_count; i++) {
            fprintf(out, ", ");
            codegen_c_node(node->data.call.args[i], out);
          }
        ...
        fprintf(out, "_r = nr_rt_len(_t); ");
      } else if (strcmp(method, "push") == 0) {
        fprintf(out, "_r = nr_rt_push(_t, ");
        if (node->data.call.arg_count > 0)
          codegen_c_node(node->data.call.args[0], out);
        else
          fprintf(out, "val_nil()");
        ...
        fprintf(out, "_r = nr_rt_pop(_t); ");
      } else {
        char p_buf[256];
        sprintf(p_buf, "%s_%s", t, method);
        if (is_function(p_buf)) {
          ...
          for (int i = node->data.call.arg_count; i < 4; i++)
            fprintf(out, ", val_nil()"); // 4 because _t is already passed
          fprintf(out, "); ");
        } else if (current_pref) {
          char p2[256];
          sprintf(p2, "%s_%s", current_pref, method);
          if (is_function(p2)) {
        ...
        fallback:
          if (is_global(method)) {
            fprintf(out,
                    "if (nr_v_%s.type == VAL_FUNC && nr_v_%s.data.func_ptr) _r "
                    "= ((Value (*)(Value, Value, Value, Value, Value, "
                    "Value))nr_v_%s.data.func_ptr)(val_nil(), _t, val_nil(), "
                    "val_nil(), val_nil(), val_nil()); else _r = val_nil(); ",
                    method, method, method);
          } else {
            fprintf(out, "_r = val_nil(); ");
          }
        }
      }
      fprintf(out, "} _r; })");
      ...
      if (current_pref) {
        ...
        if (is_function(p_buf)) {
          fprintf(out, "nr_%s(val_nil()", p_buf);
          goto args;
        }
      }
      if (is_function(n)) {
        ...
        if (current_pref) {
          ...
          if (is_global(p))
            fprintf(out, "nr_v_%s", p);
          else if (is_function(p))
            fprintf(out, "val_func(nr_%s)", p);
          else
            fprintf(out, "nr_v_%s", n);
        } else
          fprintf(out, "nr_v_%s", n);
        fprintf(out, "; Value _r; if (_f.type == VAL_FUNC && _f.data.func_ptr) "
                     "_r = ((Value (*)(Value, Value, Value, Value, Value, "
                     "Value))_f.data.func_ptr)(val_nil()");
      }
    args:
      for (int i = 0; i < node->data.call.arg_count; i++) {
        fprintf(out, ", ");
      ...
      for (int i = total_args; i < 6; i++)
        fprintf(out, ", val_nil()");
      if (!is_function(n) && (!current_pref || !is_function(p_buf)))
        fprintf(out,
                "); else { nr_rt_print(val_error(\"Function not found: %s\")); "
                "_r = val_nil(); } _r; })",
                n);
      else
        fprintf(out, ")");
  ...
  case AST_FUNC_DECL:
    if (strcmp(node->data.func_decl.name, "main") != 0)
      fprintf(out, "val_func(nr_%s)", node->data.func_decl.name);
    break;
  case AST_ERROR:
    fprintf(out, "val_error(");
    codegen_c_node(node->data.error_expr.message, out);
    fprintf(out, ".data.s)");
    break;
  case AST_BREAK:
    fprintf(out, "break");
...
static void collect_all_globals(AstNode *node) {
  ...
  case AST_PROGRAM:
    for (int i = 0; i < node->data.program.count; i++)
      collect_all_globals(node->data.program.statements[i]);
    break;
  case AST_ASSIGN:
    if (!strchr(node->data.assign.target, '.')) {
      if (!is_global(node->data.assign.target))
        global_vars[global_var_count++] = strdup(node->data.assign.target);
    }
    collect_all_globals(node->data.assign.value);
    break;
  case AST_FOR:
    if (!is_global(node->data.for_stmt.var))
      global_vars[global_var_count++] = strdup(node->data.for_stmt.var);
    if (node->data.for_stmt.alias && !is_global(node->data.for_stmt.alias))
      global_vars[global_var_count++] = strdup(node->data.for_stmt.alias);
  ...
  case AST_IMPORT: {
    const char *target = node->data.import_stmt.alias
                             ...
                             : node->data.import_stmt.path;
    if (!is_global(target))
      global_vars[global_var_count++] = strdup(target);
    for (int j = 0; j < node->data.import_stmt.symbol_count; j++) {
      if (!is_global(node->data.import_stmt.symbols[j]))
        global_vars[global_var_count++] =
            strdup(node->data.import_stmt.symbols[j]);
    }
  ...
  case AST_FUNC_DECL:
    if (node->data.func_decl.name &&
        strcmp(node->data.func_decl.name, "main") != 0 &&
        strcmp(node->data.func_decl.name, "anonymous") != 0) {
      if (!is_global(node->data.func_decl.name))
        global_vars[global_var_count++] = strdup(node->data.func_decl.name);
    }
    for (int i = 0; i < node->data.func_decl.param_count; i++) {
      if (!is_global(node->data.func_decl.params[i]))
        global_vars[global_var_count++] =
            strdup(node->data.func_decl.params[i]);
    }
  ...
  case AST_CALL:
    for (int i = 0; i < node->data.call.arg_count; i++)
      collect_all_globals(node->data.call.args[i]);
    break;
  case AST_BINARY:
  ...
  case AST_ARRAY:
    for (int i = 0; i < node->data.array.count; i++)
      collect_all_globals(node->data.array.elements[i]);
    break;
  case AST_OBJECT: {
    AstField *f = node->data.object.fields;
    while (f) {
      collect_all_globals(f->value);
      f = f->next;
    }
    break;
  }
...
void codegen_c_program(AstNode *node, FILE *out) {
  ...
  extra_link_count = 0;
  emitted_native_code_count = 0;
  generated_module_count = 0;
  collected_module_count = 0;
  collected_funcs_module_count = 0;
  global_lambda_count = 0;

  pre_collect_native(node);
  print_runtime(out);
  ...
  AstNode *m_node = NULL;
  for (int i = 0; i < node->data.program.count; i++)
    if (node->data.program.statements[i]->type == AST_FUNC_DECL &&
        strcmp(node->data.program.statements[i]->data.func_decl.name, "main") ==
            0) {
      m_node = node->data.program.statements[i];
      break;
    }
  if (m_node) {
...
char *codegen_get_links() {
  int len = 0;
  for (int i = 0; i < extra_link_count; i++)
    len += strlen(extra_links[i]) + 2;
  char *res = nr_malloc(len + 1);
  res[0] = '\0';
  for (int i = 0; i < extra_link_count; i++) {
    strcat(res, extra_links[i]);
    strcat(res, " ");
  }
  return res;
}
